...
# Number of decimal places of precision in calculations (default is 2)
PRECISION = 2
...
class Soil():
    ...
    def __init__(self, texture=None, params=None):
        ...
        if texture:  # If this class is instanced with a specific USDA soil texture.
            ...
            if not self._required_params.issubset(self.__dict__.keys()):
                missing = self._required_params.difference(
                    self.__dict__.keys())
                raise AttributeError(
                    "Missing required parameters, {list}".format(list=missing))

        else:
            raise AttributeError(
                "Must pass either a soil texture or dict of parameters")
        ...
        # Hygroscopic point in relative soil moisture [0-1]
        self.sh = self.s(self.theta(-10))
        self.nZr = None
    ...
    def theta(self, psi):
        ...
        if psi > 0:
            raise ValueError(
                "psi, {psi}, must be less than or equal to zero.".format(psi=psi))
    ...
    def s(self, theta=None, psi=None):
        ...
        try:
            ...
            raise ValueError(
                "Either theta or psi must be provided as an argument.")
    ...
    def calc_D(self, s0, units='mm/day', timestep=1):
        ...
        # This is the largest amount of Drainage possible.
        Dmax = s0 - self.sfc
